    ...
    return rx.card(
        rx.vstack(
            ...
            rx.vstack(
                ...
                rx.text('Selecciona la herramienta "Rectángulo" (□) de la barra superior. Dibuja un rectángulo que será el contenedor de tu clase.', size="2", color=color_texto_secundario, padding_left="2em"),
                spacing="1", width="100%"
            ),
            ...
            rx.vstack(
                ...
                rx.text('Usa la herramienta "Texto" (A) y escribe el nombre de la clase en la parte SUPERIOR del rectángulo.', size="2", color=color_texto_secundario, padding_left="2em"),
                spacing="1", width="100%"
            ),
            ...
            rx.vstack(
                ...
                rx.text('Usa la herramienta "Line" de la barra superior, para dibujar líneas horizontales dentro del rectángulo, dividiendo en 3 secciones: Nombre | Atributos | Operaciones.', size="2", color=color_texto_secundario, padding_left="2em"),
                spacing="1", width="100%"
            ),
            ...
            rx.vstack(
                ...
                rx.text('Usa "Texto" (A) para escribir los atributos en la sección central y las operaciones en la sección inferior.', size="2", color=color_texto_secundario, padding_left="2em"),
                rx.box(
                    rx.vstack(
...
def _contenido_leyenda() -> rx.Component:
    return rx.vstack(
        ...
        rx.grid(
            rx.card(
                rx.vstack(
                    ...
                    rx.text('Selecciona la herramienta "Texto" (A) de la barra superior para escribir nombres de clases, atributos y operaciones.', size="2", color=color_texto_gris),
                    rx.divider(margin_y="0.5em"),
                    rx.center(
            ...
            rx.card(
                rx.vstack(
                    ...
                    rx.text('Selecciona la herramienta "Flecha" (→) de la barra superior para dibujar las relaciones entre las clases del diagrama.', size="2", color=color_texto_gris),
                    rx.divider(margin_y="0.5em"),
                    rx.center(
            ...
            rx.card(
                rx.vstack(
                    ...
                    rx.text("Tras seleccionar una flecha, elige el tipo de punta (Arrowheads) desde el panel izquierdo.", size="2", color=color_texto_gris),
                    rx.divider(margin_y="0.5em"),
                    rx.center(
            ...
            rx.card(
                rx.vstack(
                    ...
                    rx.text("Tras haber pintado una línea, cambia entre línea sólida y discontinua (Stroke style) en el panel izquierdo.", size="2", color=color_texto_gris),
                    rx.divider(margin_y="0.5em"),
                    rx.hstack(
                        rx.vstack(
        ...
        rx.text("Todas las relaciones se dibujan usando la herramienta Flecha (→) de la barra superior. Después, ajusta el tipo de línea y la punta desde el panel derecho.", size="2", color=color_texto_gris, margin_bottom="1em"),

        rx.grid(
            seccion_relacion("Asociación", "Relación genérica entre dos clases.", [
                'Selecciona la herramienta "Flecha" (→) de la barra superior.',
                "Dibuja una línea de una clase a otra.",
                'En el panel izquierdo, asegúrate de que la raya (Stroke style) sea de tipo solid (—).',
            ...
            seccion_relacion("Agregación", "Relación 'tiene un' (el todo puede existir sin las partes).", [
                ...
                'Raya de tipo solid (—).',
                'Final: DIAMANTE hueco(◇) desde "Arrowheads".',
            ], color_exito),
            rx.card(
                rx.vstack(
                    rx.hstack(
                    ...
                    rx.vstack(
                        ...
                        rx.hstack(rx.box(width="6px", height="6px", border_radius="full", background_color=color_primario, flex_shrink="0", margin_top="6px"), rx.text('Raya solid (—).', size="2", color=color_texto_secundario), align="start", spacing="2"),
                        rx.hstack(rx.box(width="6px", height="6px", border_radius="full", background_color=color_primario, flex_shrink="0", margin_top="6px"), rx.text('Final: DIAMANTE relleno (◆) desde "Arrowheads".', size="2", color=color_texto_secundario), align="start", spacing="2"),
                        spacing="2", width="100%"
                    ),
                    spacing="3", width="100%", align_items="start"
                ),
